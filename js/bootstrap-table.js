    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            if (this.options.cardView) {
                html.push(sprintf('<td onclick="addRowHandlers()" colspan="%s">', this.header.fields.length));
            }
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    text = ['<td onclick="addRowHandlers()" class="bs-checkbox">',
                        '<input' +
                            sprintf(' data-index="%s"', i) +
                    ...
                    text = that.options.cardView ?
                        ['<div class="card-view">',
                            ...
                            '</div>'].join('') :
                        [sprintf('<td onclick="addRowHandlers()" %s %s %s>', id_, class_, style),
                            value,
                            '</td>'].join('');
                }
        ...
        if (!html.length) {
            html.push('<tr class="no-records-found">',
                sprintf('<td onclick="addRowHandlers()" colspan="%s">%s</td>', this.header.fields.length, this.options.formatNoMatches()),
                '</tr>');
        }
